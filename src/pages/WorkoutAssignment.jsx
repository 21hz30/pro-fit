...
import { getCoachExercises, getCoachPlans, saveDraft } from '../services/workoutService.js';
...
export function WorkoutAssignment({ requestedTraineeId }) {
  ...
  useEffect(() => {
    if (!plan.traineeId) return;
    let active = true;
    getCoachPlans(client, plan.traineeId).then((rows) => { if (active) setSavedPlans(client.isLocal ? rows : rows.filter((row) => row.status === 'draft')); }).catch((error) => { if (active) setToast({ message: error.message, tone: 'error' }); });
    return () => { active = false; };
